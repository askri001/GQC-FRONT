...
export interface ExpertFormDialogData {
  prestataire?: Prestataire;
  isEdit: boolean;
}

@Component({
  selector: 'app-expert-form-dialog',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
    ...
    <div class="dialog-container">
      <div class="dialog-header">
        <h2 mat-dialog-title>
          ...
          {{ data.isEdit ? 'Modifier Expert' : 'Nouvel Expert' }}
        </h2>
...
export class ExpertFormDialogComponent implements OnInit {
  form!: FormGroup;
  isLoading = false;
  specialites = PRESTATAIRE_SPECIALITES['EXPERT' as TypePrestataire];

  constructor(
    private fb: FormBuilder,
    public dialogRef: MatDialogRef<ExpertFormDialogComponent>,
    @Inject(MAT_DIALOG_DATA) public data: ExpertFormDialogData
  ) {}
  ...
  onSubmit(): void {
    if (this.form.valid) {
      ...
      const prestataireData: Partial<Prestataire> = {
        ...formValue,
        typePrestataire: 'EXPERT' as TypePrestataire
      };
